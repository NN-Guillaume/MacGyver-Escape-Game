...
class Level:
    ...
    def generate(self):
        """ this method enable the level creation thanks to the file 'level' """
    ...
    def display(self, window):
        """ this method display the level thanks to the structure's list sended by the 'generate' method """
...
class Charac:
    ...
    def __init__(self, right, left, up, down, level):
        ...
        #self.harvest_items = 0 #compteur d'items rammassés !
    ...
    def move(self, direction):
        ...
        if direction == 'right':
            ...
            if self.case_x < (nbr_side_sprite - 1):
                #On vérifie que la case de destination n'est pas un mûr !
                if self.level.structure[self.case_y][self.case_x + 1] not in obstacles:
        ...
        if direction == 'up':
            if self.case_y > 0:
                if self.level.structure[self.case_y - 1][self.case_x] not in obstacles:
                    self.case_y -= 1
        ...
        #if suis-je sur une case avec un objet ? 
